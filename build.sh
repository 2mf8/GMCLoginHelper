...
for PLATFORM in $PLATFORMS; do
  ...
  CMD="GOOS=${GOOS} GOARCH=${GOARCH} go build -v -o ${BIN_FILENAME} github.com/2mf8/GMCLoginHelper"
  echo $CMD
...
if [ -n "$1" ]; then
  rm "$RELEASE_DIR/gmc_login_helper-$1.zip"
  CMD="zip -r $RELEASE_DIR/gmc_login_helper-$1.zip $OUTPUT_DIR -x \".*\" -x \"__MACOSX\""
  echo $CMD
  eval $CMD
fi